...
def JEH(eleve, etude):
    ...
    montant_total = sum(
        assignment.retribution_brute_totale() for assignment in queryset
    )

    return f"{nb_total_JEH} {JEH} pour un montant de {montant_total:.2f}€ HT"


@register.filter(name="JEHpreloaded")
def JEHpreloaded(eleve, etude):
    nb_total_JEH = getattr(eleve, "precomputed_JEH_count", 0)
    montant_total = getattr(eleve, "precomputed_montant_total", 0.0)

    JEH_label = "JEH" if nb_total_JEH <= 1 else "JEHs"
    return f"{nb_total_JEH} {JEH_label} pour un montant de {montant_total:.2f}€ HT"
...
def order_dict_by_keys(dictionary):
    ...
    ordered_dict = [
        (key, dictionary[key]) for key in full_ordered_keys if key in dictionary
    ]
